...
YOLO::YOLO(Net_config config)
{
	...
	{
		input_names.push_back(_strdup(ort_session->GetInputNameAllocated(i , allocator).get()));
	...
	{
		output_names.push_back(_strdup(ort_session->GetOutputNameAllocated(i , allocator).get()));
...
void YOLO::detect(Mat& frame)
{
	...
	Value input_tensor_ = Value::CreateTensor<float>(
		allocator_info, input_image_.data(), input_image_.size(), input_shape_.data(), input_shape_.size());

	vector<Value> ort_outputs = ort_session->Run(
		RunOptions{ nullptr }, input_names.data(), &input_tensor_, 1, output_names.data(), output_names.size());   // ��ʼ����
